	...
	case "$action" in
		block)
			...
			if ! wget -q "$download_url" -O "${country_code,,}.zone"; then
				echo "错误：下载 $country_codeIPゾーンファイルが失敗しました"
				exit 1
			fi
		...
		allow)
			# 許可されている国の IPSET が存在しない場合は作成します。
			if ! ipset list "$ipset_name" &> /dev/null; then
				ipset create "$ipset_name" hash:net
			fi
...
iptables_panel() {
  ...
  while true; do
		  ...
		  case $sub_choice in
			  ...
			  15)
				  read -e -p "ブロックされている国コードを入力してください (例: CN、US、JP):" country_code
			...
			rm -rf /etc/fail2ban
			  ...
			  case $sub_choice in
				  ...
				  22)
					  ...
					  echo -e "cf バックエンド ドメイン名の概要ページの右下に移動して取得します。${gl_huang}区域ID${gl_bai}"
					  echo "https://dash.cloudflare.com/login"
					  echo "--------------"
		  ...
		  while true; do
			...
			case "$sys_choice" in
			  ...
			  45)
				...
				;;
			  46)
				send_stats "Windowsサーバー16を再インストールします"
				dd_xitong_2
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}75.  ${color75}Melody音乐精灵                      ${gl_kjlan}76.  ${color76}オンライン DOS 古いゲーム"
